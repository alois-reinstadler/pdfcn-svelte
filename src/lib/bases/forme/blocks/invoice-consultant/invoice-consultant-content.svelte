<script lang="ts">
	...
	const styles = {
		...
		companyInfo: { flex: 1 },
		headerRow: { alignItems: 'flex-start', borderBottomColor: theme.colors.primary, borderBottomStyle: 'solid', borderBottomWidth: 2, flexDirection: 'row', justifyContent: 'space-between', marginBottom: theme.spacing.sectionGap, paddingBottom: theme.spacing.componentGap },
		hoursBadge: { alignSelf: 'flex-start', backgroundColor: theme.colors.primary, borderRadius: theme.primitives.borderRadius.sm, color: theme.colors.primaryForeground, paddingHorizontal: 12, paddingVertical: 8 },
		hoursBox: { flex: 1, paddingRight: 24 },
		...
		<View style={styles.page}>
			<View style={styles.headerRow}>
				<View style={styles.companyInfo}>
					<Text variant="xl" weight="bold" noMargin>{data.companyName}</Text>
					<Text variant="sm" color="mutedForeground" noMargin>{data.subtitle}</Text>
					<Text variant="xs" color="mutedForeground" noMargin>{data.companyAddress}</Text>
				</View>
				<View style={styles.invoiceInfo}>
					<Text variant="xs" color="mutedForeground" transform="uppercase" noMargin>Invoice</Text>
					<Text variant="lg" weight="bold" noMargin>{data.invoiceNumber}</Text>
					<Text variant="xs" color="mutedForeground" noMargin>{data.invoiceDate}</Text>
					<Text variant="xs" color="mutedForeground" noMargin>Due: {data.dueDate}</Text>
				</View>
			</View>
			{#if data.projectRef}
				<View style={styles.projectRef}>
					<Text variant="xs" weight="semibold" color="mutedForeground" noMargin>Project Reference:</Text>
					<Text variant="xs" weight="bold" noMargin>{data.projectRef}</Text>
				</View>
			{/if}
			<View style={styles.partiesRow}>
				...
				<View style={styles.partyColumn}>
					...
					<Text variant="xs" noMargin color="mutedForeground">{data.client.email}</Text>
				</View>
			</View>
			<Table variant="line">
			...
			<Section noWrap style={styles.summaryRow}>
				<View style={styles.hoursBox}>
					<View style={styles.hoursBadge}><Text style={{ color: theme.colors.primaryForeground, fontSize: 9, fontWeight: 'bold' }} noMargin>Total Hours: {data.summary.totalHours}</Text></View>
					<Text variant="xs" color="mutedForeground" style={{ marginTop: 8 }}>Payment: {data.paymentTerms.method}</Text>
				</View>
			...
			{#if data.notes}<View style={styles.calloutNote}><Text variant="xs" color="mutedForeground">{data.notes}</Text></View>{/if}
		</View>
	</Page>
</Document>
