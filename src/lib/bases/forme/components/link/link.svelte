<script lang="ts">
	...
	import PDFLink from '$lib/bases/forme/lib/Link.svelte';
	...
	const mergedStyle = $derived.by(() => {
		...
		const args: Record<string, unknown>[] = [variantMap[variant]];
		if (underline && underline in underlineMap) {
			args.push(underlineMap[underline]);
		}
		if (align || color) {
...
<PDFText style={mergedStyle}>
	<PDFLink {href} style={mergedStyle}>
		{@render children?.()}
	</PDFLink>
</PDFText>
