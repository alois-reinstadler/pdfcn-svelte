<script lang="ts">
	...
	const mergedStyle = $derived.by(() => {
		...
		const args: Record<string, unknown>[] = [headingStyles[safeLevel]];
		if (weight && weight in weightMap) {
			args.push(weightMap[weight]);
		}
		if (tracking && tracking in trackingMap) {
			args.push(trackingMap[tracking]);
		}
		if (transform && transform in transformMap) {
			args.push(transformMap[transform]);
		}
		if (noMargin) {
			args.push(styles.noMargin);
		}
		if (align || color) {
		...
		return mergeFormeStyles(...args, style ?? undefined);
	});
</script>
