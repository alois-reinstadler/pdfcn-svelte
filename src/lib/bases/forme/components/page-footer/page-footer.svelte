...
{#snippet footerChildren()}
	...
	{:else if variant === 'centered'}
		{#if leftText}<PDFText style={colorize(styles.textCenteredVariant)}>{leftText}</PDFText>{/if}{#if rightText}<PDFText style={colorize(styles.textCenteredVariant)}>{rightText}</PDFText>{/if}
	{:else if variant === 'three-column'}
		<View style={styles.threeColumnLeft}>{#if leftText}<PDFText style={colorize(styles.companyName)}>{leftText}</PDFText>{/if}{#if address}<PDFText style={styles.textLeft}>{address}</PDFText>{/if}</View>
