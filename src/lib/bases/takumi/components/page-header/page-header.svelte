<script lang="ts">
	...
	const createPageHeaderStyles = (t: PdfcnTheme) => {
		...
		return {
			brandedContainer: { alignItems: 'center', backgroundColor: c.primary, borderRadius: borderRadius.sm, display: 'flex', flexDirection: 'column', padding: spacing[6] },
			centeredContainer: { alignItems: 'center', borderBottomColor: c.border, borderBottomStyle: 'solid', borderBottomWidth: spacing[0.5], display: 'flex', flexDirection: 'column', paddingBottom: spacing[4] },
			contactInfo: { color: c.mutedForeground, fontFamily: body.fontFamily, fontSize: t.primitives.typography.xs, marginTop: spacing[0.5], textAlign: 'right' },
			logoContainer: { height: 48, marginRight: spacing[4], width: 48 },
			logoContent: { display: 'flex', flex: 1, flexDirection: 'column' },
			logoLeftContainer: { alignItems: 'center', borderBottomColor: c.border, borderBottomStyle: 'solid', borderBottomWidth: spacing[0.5], display: 'flex', flexDirection: 'row', paddingBottom: spacing[4] },
			logoRightContainer: { alignItems: 'center', borderBottomColor: c.border, borderBottomStyle: 'solid', borderBottomWidth: spacing[0.5], display: 'flex', flexDirection: 'row', justifyContent: 'space-between', paddingBottom: spacing[4] },
			logoRightContent: { display: 'flex', flex: 1, flexDirection: 'column' },
			logoRightLogoContainer: { height: 48, marginLeft: spacing[4], width: 48 },
			minimalContainer: { alignItems: 'center', borderBottomColor: c.primary, borderBottomStyle: 'solid', borderBottomWidth: spacing[1], display: 'flex', flexDirection: 'row', justifyContent: 'space-between', paddingBottom: spacing[3] },
			minimalLeft: { flex: 1 }, minimalRight: { alignItems: 'flex-end' },
			rightSubText: { color: c.mutedForeground, fontFamily: body.fontFamily, fontSize: t.primitives.typography.xs, marginTop: spacing[1], textAlign: 'right' },
			rightText: { color: c.foreground, fontFamily: body.fontFamily, fontSize: body.fontSize, fontWeight: fontWeights.medium, textAlign: 'right' },
			simpleContainer: { alignItems: 'flex-start', borderBottomColor: c.border, borderBottomStyle: 'solid', borderBottomWidth: spacing[0.5], display: 'flex', flexDirection: 'row', justifyContent: 'space-between', paddingBottom: spacing[4] },
			simpleLeft: { display: 'flex', flex: 1, flexDirection: 'column' }, simpleRight: { alignItems: 'flex-end', display: 'flex', flexDirection: 'column' },
			subtitle: { color: c.mutedForeground, fontFamily: body.fontFamily, fontSize: body.fontSize, lineHeight: body.lineHeight, marginTop: spacing[1] },
			subtitleBranded: { color: c.primaryForeground, marginTop: spacing[1] }, subtitleCentered: { textAlign: 'center' },
			title: { color: c.foreground, fontFamily: heading.fontFamily, fontSize: heading.fontSize.h3, fontWeight: fontWeights.bold, lineHeight: heading.lineHeight, marginBottom: 0 },
			titleBranded: { color: c.primaryForeground }, titleCentered: { textAlign: 'center' }, titleMinimal: { fontSize: heading.fontSize.h3, fontWeight: fontWeights.bold },
			twoColumnContainer: { alignItems: 'flex-start', borderBottomColor: c.border, borderBottomStyle: 'solid', borderBottomWidth: spacing[0.5], display: 'flex', flexDirection: 'row', justifyContent: 'space-between', paddingBottom: spacing[4] },
			twoColumnLeft: { display: 'flex', flex: 1, flexDirection: 'column' }, twoColumnRight: { alignItems: 'flex-end', display: 'flex', flexDirection: 'column' }
		};
	};
...
<View wrap={!noWrap} style={containerStyle}>
	{#if variant === 'branded' || variant === 'centered'}
		...
		{#if subtitle}<PDFText style={[styles.subtitle, variant === 'branded' ? styles.subtitleBranded : styles.subtitleCentered]}>{subtitle}</PDFText>{/if}
	{:else if variant === 'logo-right'}
		<View style={styles.logoRightContent}><PDFText style={titleStyle}>{title}</PDFText>{#if subtitle}<PDFText style={styles.subtitle}>{subtitle}</PDFText>{/if}</View>
	...
	{:else if variant === 'logo-left'}
		...
		<View style={styles.logoContent}><PDFText style={titleStyle}>{title}</PDFText>{#if subtitle}<PDFText style={styles.subtitle}>{subtitle}</PDFText>{/if}</View>
