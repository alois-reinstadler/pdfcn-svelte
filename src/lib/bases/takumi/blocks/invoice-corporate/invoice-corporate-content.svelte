...
{#snippet logo()}
	<PdfImage src={data.logo ?? '/favicon.png'} width={56} height={56} style={{ margin: 0 }} />
{/snippet}
...
<Document title={`Invoice ${data.invoiceNumber}`}>
	<Page size="A4" style={pageStyle}>
		...
		<Table variant="bordered"><TableHeader><TableRow header><TableCell text="Description" /><TableCell align="center" text="Qty" /><TableCell align="right" text="Unit Price" /><TableCell _last align="right" text="Amount" /></TableRow></TableHeader>
			<TableBody>{#each data.items as item}<TableRow><TableCell text={item.description} /><TableCell align="center" text={`${item.quantity}`} /><TableCell align="right" text={`$${item.unitPrice.toLocaleString()}`} /><TableCell _last align="right" text={`$${(item.quantity * item.unitPrice).toLocaleString()}`} /></TableRow>{/each}</TableBody>
		</Table>
		<View style={{ backgroundColor: theme.colors.muted, borderRadius: theme.primitives.borderRadius.md, marginTop: 20, padding: 16 }}>
			<View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}><View style={{ width: 260 }}><KeyValue size="md" dividerThickness={1} dividerColor="border" divided items={[
				{ key: 'Subtotal', value: `$${data.summary.subtotal.toLocaleString()}` }, { key: 'Tax (8%)', value: `$${data.summary.tax.toFixed(2)}` },
